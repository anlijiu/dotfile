...
async function start() {
    const res = await fetch('https://tianqi.moji.com/weather/china/beijing/chaoyang-district');
    ...
    await downloadIfNotExist({uri:pm25ImgElem.attr('src'), img_path: pm25ImgPath });
    ...
    await downloadIfNotExist({uri:weather_img});
    ...
    var forecast = await Promise.all($('.forecast').children('ul').map(async (index, elem) => {
      elem = $(elem);
      ...
      let uri = secondLi.find('img').attr('src');
        console.log(" uri in map :", uri)
      await downloadIfNotExist({uri});
...
async function downloadIfNotExist({uri, img_path=imgPath}) {

    console.log("downloadIfNotExist   uri :", uri, " img_path:", img_path)
  ...
  try {
    ...
    //捕获异常
    await download(uri, img_path);
  }
}

function tryWriteFile(info) {
    console.log("tryWriteFile   info:", info)
  try {
